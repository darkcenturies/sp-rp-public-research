...
static int __cdecl InstallSharedLookups() {
    ...
    while(cursor<0x80000000u && VirtualQuery(reinterpret_cast<void*>(cursor),&region,sizeof(region))) {
        ...
        if(region.State==MEM_COMMIT && region.Type==MEM_PRIVATE && (region.Protect&0xf0) && !(region.Protect&(PAGE_GUARD|PAGE_NOACCESS))) {
            ...
            for(size_t n=3;n+14<=region.RegionSize;++n) {
                uintptr_t value;memcpy(&value,bytes+n,4);
                if(value!=originalModels) continue;
                unsigned jumpOffset=4;
                if(bytes[n+4]==0xe8 && bytes[n+9]==0xe9) {
                    int callDisplacement;memcpy(&callDisplacement,bytes+n+5,4);
                    uintptr_t helper=reinterpret_cast<uintptr_t>(bytes+n+9)+callDisplacement;
                    MEMORY_BASIC_INFORMATION helperMemory={};
                    if(!VirtualQuery(reinterpret_cast<void*>(helper),&helperMemory,sizeof(helperMemory)) || helperMemory.AllocationBase!=GetModuleHandleW(L"$fastman92limitAdjuster.asi")) continue;
                    jumpOffset=9;
                }
                if(bytes[n+jumpOffset]!=0xe9) continue;
                if(bytes[n-3]!=0x8b && bytes[n-3]!=0x89 && bytes[n-3]!=0x39) continue;
                if((bytes[n-2]&0xc7)!=4 || (bytes[n-1]&0xc7)!=0x85) continue;
                int displacement;memcpy(&displacement,bytes+n+jumpOffset+1,4);
                uintptr_t continuation=reinterpret_cast<uintptr_t>(bytes+n+jumpOffset+5)+displacement;
                bool known=false;
                for(unsigned offset=0;offset<0xdcf;offset+=5) {
